    ...
    public partial class FrmPermissionList : Form
    {
        ...
        private void FrmPermissionList_Load(object sender, EventArgs e)
        {
            ...
            if (!UserStatic.isAdmin)
            {
                ...
                btnDisapprove.Hide();
                btnDelete.Hide();
                btnClose.Location = new Point(670, 40);
            }
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (radioStart.Checked)
                list = list.Where(x => x.StartDate > Convert.ToDateTime(dateStart.Value) &&
                x.StartDate < Convert.ToDateTime(dateFinish.Value)).ToList();
            if (radioEnd.Checked)
                list = list.Where(x => x.EndDate > Convert.ToDateTime(dateStart.Value) &&
                x.EndDate < Convert.ToDateTime(dateFinish.Value)).ToList();
        ...
        {
            DialogResult result = MessageBox.Show("Are your sure to delete this permission?", "Warning", MessageBoxButtons.YesNo);
